...
class Game:
    ...
    def post_json(self, url, body, timeout_sec, tag):
        ...
        secret = self.runtime_settings.get(
            "embedding_api_key" if tag == "EMB" else "chat_api_key",
            "",
        )
        ...
        if self.debug_for_tag(tag):
            ...
            print(
                "base_url="
                + (self.llm_base_url() if tag != "EMB" else self.emb_base_url())
            )
            print(
                "model="
                + (self.llm_model() if tag != "EMB" else self.emb_model())
            )
            ...
            print(
                ...
                "auth_header="
                + ("Authorization" if secret else "none")
            )
            ...
            print(
                ...
                f"messages="
                f"{len(body.get('messages', [])) if isinstance(body.get('messages'), list) else 0} "
    ...
    def llm_model(self):
        return (
            self.runtime_settings.get("chat_model")
    ...
    def chat_provider(self):
        return (
            self.runtime_settings.get("chat_provider")
    ...
    def companion_banter_prompt(self):
        return (
            ...
            "通常は1〜3人だけ反応する。"
            "全員参加は稀。"
            "requested_companions が存在する場合、その仲間は必ず1回発言する。"
            "全員に台詞を与えようとしない。"
            "興味を持たない人物は黙っていてよい。"
