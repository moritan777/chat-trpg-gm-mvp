...
# -*- coding: utf-8 -*-
"""
Chat-style TTRPG GM MVP v2.15.12
...
VERSION = "v2.15.12 [companion-address]"
...
class Game:
    ...
    def companion_banter_prompt(self):
        return (
            ...
            "身近な現実も気にし、仲間への注意、確認、ツッコミ、気遣い、便乗、沈黙を選べる。安全指導だけに固定しない。\n"
            "\n"
            "【ニコ】\n"
            "分析より目についたものや連想を口にしやすい。事件仮説に限らず、妙な例え、疑問、使い道、"
            "身体感覚、急な脱線を仲間へ投げてもよい。正しい必要も有益である必要もなく、冗談だけに固定しない。\n"
            "\n"
            "【ピピ】\n"
            "論理より感情や人間関係へ反応しやすい。怖がるだけでなく、景色や物、匂いや汚れ、疲れ、"
            "仲間へ反応し、心配する、同意を求める、妙な話へ乗る、甘えることがある。誰か一人への依存や事件分析のまとめ役に固定しない。\n"
            "\n"
            "【関心対象】\n"
            ...
            "\n"
            "【仲間会話】\n"
            "話題はGMの最後の一文に従属せず、Discoverable開示時も原因や仮説を述べなくてよい。"
            "場面への感想と仲間への働きかけは、どちらも自然な発言である。最初の仲間も独り言だけでなく、別の仲間へ言葉や行動を向けてよい。\n"
            "名前を呼ぶ、質問や同意を求める、注意する、助けを頼む、物を見せる、ツッコむ、茶化す、袖を掴むなど、反応を誘う発言から始められる。"
            "人物関係を作る発言は事件解決に役立たなくてもよい。\n"
            "複数行では、GMへの独立コメントを並べるより、働きかけと短い応答が自然ならその形を選んでよい。"
            "相手は自然なら短く応答でき、そこで終えてよい。独り言、沈黙、無視が自然なら会話を強制しない。\n"
            "仲間同士の話題からGMへ戻る必要も、同じ発見へ全員が別コメントを出す必要もない。掛け合いは必須ではない。\n"
            "\n"
            "【事実境界】\n"
    ...
    def render_table_turn(self, notes, it, res, ev, st):
        ...
        packet = {
            ...
            "instructions": [
                ...
                "仲間発言は0〜3行。最初の仲間も別の仲間へ話しかけてよく、複数行では独立コメントの列より短い働きかけと応答を選べる。必要な人物だけ話し、同じ人物が短く再応答してよい。全員を一度ずつ出す必要はない。",
